...
void HW04obissiam_1App::setup()
{
	...
	for (int j = 0; j < appHeight; j++)	{
		for(int i = 0; i < appWidth; i++)	{
			...
			if(dataArray[offset + 0] != 0 && dataArray[offset + 0] != 255)	{
	...
	//drawMap(currentNode->right_->right_, true, appWidth/2, 0, 0.025);
	drawMap(currentNode->left_->right_, true, 0, 0, 0.025);
...
void HW04obissiam_1App::drawMap(Node* root, bool zoom, int x, int y, double magicNumber)	{
	...
	for(int j = 0; j < appHeight/2; j++)	{
		for(int i = 0; i < appWidth/2; i++)	{
			...
			info[offset + 0] = dataArray[offsetZoom + 0];
			info[offset + 1] = dataArray[offsetZoom + 1];
			info[offset + 2] = dataArray[offsetZoom + 2];
		}
	}
	if (zoom)	{
	...
	for (int j = 0; j < appHeight; j++)	{
		for(int i = 0; i < appWidth; i++)	{
			...
			if(!(dataArray[offset + 0] == 0 && dataArray[offset + 1] == 0  && dataArray[offset + 1] == 0) && 
				...
				!(dataArray[offset + 0] == 0 && dataArray[offset + 1] == 255 && dataArray[offset + 1] == 0))	{
...
void HW04obissiam_1App::keyDown( KeyEvent event) {
	if (leftClicked)	{
		...
		if (event.getChar() == 'p')		{
			...
			drawMap(currentNode, true, appWidth/2, 0, 0.025);
		}
		if (event.getChar() == 'o')		{
